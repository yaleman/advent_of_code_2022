...
fn move_items(piles: &mut Piles, count: usize, from: usize, to: usize) -> Piles {
    ...
    let from_vec = piles.get(&(from)).unwrap().to_vec();
    let mut to_vec = piles.get(&(to)).unwrap().to_vec();

    let (from_vec, extendobit) = from_vec.split_at(from_vec.len() - count);
    to_vec.extend(extendobit);
    // for _ in 0..count {
    //     to_vec.push(from_vec.pop().unwrap());
    // }
    piles.insert(from, from_vec.to_vec());
